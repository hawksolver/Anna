...
function resizeCanvas() {
  ...
  drawCenteredObject(); // Reposiciona o objeto após o redimensionamento
}

// Desenha o objeto centralizado
function drawCenteredObject() {
  const centerX = canvas.width / 2;
  const centerY = canvas.height / 2;
  const objectSize = 50; // Tamanho do objeto (ajuste conforme necessário)

  ctx.clearRect(0, 0, canvas.width, canvas.height); // Limpa o canvas
  ctx.fillStyle = "red"; // Cor do objeto
  ctx.fillRect(
    centerX - objectSize / 2,
    centerY - objectSize / 2,
    objectSize,
    objectSize
  ); // Desenha o objeto
}
...
setTimeout(() => {
  document.body.classList.add("zoom-out"); // Adiciona a classe para o efeito de zoom

  // Aguarda o tempo da transição antes de redirecionar
  setTimeout(() => {
    window.location.href = "./telas/bnvd.html"; // Redireciona para a próxima página
